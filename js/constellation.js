...
import { smoothstep, clamp, isMobile } from './util.js';
...
class AgentNode {
  constructor(def, scene, labelRoot) {
    ...
    this.flare = 0;     // dispatch flare 0..1
    this.working = 0;   // working pulse intensity 0..1
    this.dock = null;   // docking target (Tier 5)
...
export class Constellation {
  ...
  update(dt, t, sizePx) {
    ...
    for (const node of this.nodes) {
      ...
      if (node.dock) node.dock.apply(pos, dt);
      ...
      const glowOpacity = baseGlow + node.flare * 0.9 + node.working * 0.5;
      node.glow.material.opacity = glowOpacity * (0.45 + 0.55 * front);
      const glowScale = 1.15 + node.flare * 0.9 + node.working * 0.25
        + 0.06 * breathe;
      node.glow.scale.setScalar(glowScale);
      node.avatar.scale.setScalar(0.46 + node.flare * 0.22);
